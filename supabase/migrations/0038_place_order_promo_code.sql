-- 0038_place_order_promo_code.sql  (#3 Promotions — step 2)
-- place_order now accepts an optional promo CODE: it re-validates the code server-
-- side via validate_promo (authoritative), applies the discount, and records the
-- redemption (so per-user / global caps hold). The legacy p_promo boolean (blanket
-- 15%) is kept for backward compatibility but only used when no code is supplied.

drop function if exists public.place_order(uuid, jsonb, text, text, uuid, boolean, int, numeric, text, text);
...
create or replace function public.place_order(
  ...
  p_branch_slug text default null, p_promo_code text default null
...
begin
  ...
  -- Discount: a promo CODE takes precedence over the legacy blanket toggle.
  ...
  -- Record the promo redemption (enforces global / per-user caps going forward).
...
revoke all on function public.place_order(uuid, jsonb, text, text, uuid, boolean, int, numeric, text, text, text) from public;
grant execute on function public.place_order(uuid, jsonb, text, text, uuid, boolean, int, numeric, text, text, text) to authenticated, service_role;
