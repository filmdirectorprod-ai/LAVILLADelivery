-- La Villa — Manager confirmation flow (sub-project B), part 1.
-- Re-defines place_order so a new order lands as `pending` (awaiting the gérant's
-- confirmation) with NO driver assigned, instead of going straight to `preparing`
-- with a random driver. All money/loyalty math is unchanged. The customer is told
-- the order was received and is awaiting confirmation.
...
begin
  ...
  -- 6) Insert order as PENDING (awaiting gérant confirmation), no driver.
  ...
  values (v_code, p_user, 'pending', p_mode, p_address, p_zone,
  ...
  -- 10) Notify: received, awaiting confirmation.
