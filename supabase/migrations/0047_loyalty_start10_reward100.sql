-- 0047_loyalty_start10_reward100.sql
-- Loyalty tweaks:
--   • New customers start with a 10-pt welcome bonus (+ ledger entry).
--   • Every order earns at least 10 points (greatest(10, floor(total))).
--   • A new redemption tier 100 pts → 10 DH (kept ratio 10 pts = 1 DH).
--   • Realtime: profiles + loyalty_ledger published so the customer's points and
...
begin
  ...
  values (new.id, coalesce(new.raw_user_meta_data->>'full_name', ''), 10, 'Gourmand')
...
-- place_order: min 10 pts earned + accept the 100→10 redemption tier
create or replace function public.place_order(p_user uuid, p_items jsonb, p_mode text, p_address text, p_zone uuid, p_promo boolean, p_redeem_pts integer, p_redeem_dh numeric, p_phone text DEFAULT NULL::text, p_branch_slug text DEFAULT NULL::text, p_promo_code text DEFAULT NULL::text)
...
-- Realtime so loyalty points + activity update live in the customer app.
